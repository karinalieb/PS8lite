...
mtry <- 2:5 #NOTE: I wanted to use all 36 predictors (mtry <- 2:36), but it took way too long to compute and also crashed my RStudio. So I'm sticking with 2-5 predictors just so it will run! 
...
#We see that using `mtry = 5` yielded the lowest estimate of `RMSE` model error on new independent test data. This is the optimal value.
```
...
submission_rf <- test %>%
  ...
  select(Id, SalePrice)
write_csv(submission_rf, "data/kl_submission_rf.csv")
```
